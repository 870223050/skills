...
Unless the user explicitly requests another language, create new skills in Chinese by default.

- Write the `description` field in Chinese.
- Write the SKILL.md body, section headings, examples, and procedural guidance in Chinese.
- Write `agents/openai.yaml` user-facing fields such as `display_name`, `short_description`, and `default_prompt` in Chinese.
- If `init_skill.py` generates an English template, rewrite it to Chinese during the editing step before considering the skill complete.
...
**Language Guidelines:** Default to Chinese for newly created skills unless the user explicitly requests another language.
...
新建或修改 skills 后，都要同步整个 `skills` 文件夹到对应的 GitHub 地址并更新 `README.md`；对应的 skill Git 地址为 `https://github.com/870223050/skills.git`。注意：提交注释和 `README.md` 默认采用中文描述。
